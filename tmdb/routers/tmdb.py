...
APIKEY = "1b91b4ea96286f180cac78a0d0b2d40f"
...
async def search_movie_title(title: str = Path(description="Movie title", min_length=1, example="Back to the future")):
    try:
        ...
        search_url = f"{URL}/search/movie?api_key={APIKEY}&include_adult=false&language=es-ES&page=1&query={title_url_encoded}"
...
async def get_movie_info(movie_id: int = Path(description="Movie id in TMDB", min=1, example=165)):
    try:
        get_url = f"{URL}/movie/{movie_id}?api_key={APIKEY}"
